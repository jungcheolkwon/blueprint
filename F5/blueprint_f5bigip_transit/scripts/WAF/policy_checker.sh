...
ip=$(az network public-ip show -n $lb_name -g $rg --query ipAddress -o tsv)
	...
	curl -sk -H "X-F5-Auth-Token: $token" -X GET https://$ip:8443/mgmt/tm/asm/policy-templates | jq -r .
	...
	curl -sk -H "X-F5-Auth-Token: $token" -X GET https://$ip:9443/mgmt/tm/asm/policy-templates | jq -r .
