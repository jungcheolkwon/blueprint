...
then
	...
	curl -sk -H "Content-Type: test/x-yaml" -H "X-F5-Auth-Token: $token" -X POST --data-binary @parent_policy.json https://$ip:8443/mgmt/tm/asm/policies | jq -r .id > hashp
	#./phash.sh
...
then
	...
	curl -sk -H "Content-Type: test/x-yaml" -H "X-F5-Auth-Token: $token" -X POST --data-binary @child_policy.json https://$ip:8443/mgmt/tm/asm/policies | jq -r .id > hashc
	#./chash.sh
